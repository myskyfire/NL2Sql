# -*- coding: utf-8 -*-
"""
重新生成L6-L9扩展测试用例并追加到training_test_cases.txt
确保UTF-8编码正确
"""
...
# ===== L6: 时间查询与日期处理 (50个) =====
# 关键规则: 
# - 具体日期 -> WHERE DATE(created_at) = 'YYYY-MM-DD'
# - 时间范围 -> WHERE created_at >= ... AND created_at <= ...
# - 按天/月统计 -> GROUP BY DATE_FORMAT(..., '%Y-%m-%d')
...
# 写入新文件（保留原始L1-L5，替换损坏的L6-L9）
with open('training_test_cases_new.txt', 'w', encoding='utf-8', newline='') as f:
    # 写入原始内容
    f.writelines(original_lines)
    
    # 写入扩展内容
    f.write(extended_cases.strip() + '\n')

print("✅ Successfully regenerated training_test_cases.txt with proper UTF-8 encoding")
print(f"   Original lines (L1-L5): {len(original_lines)}")
print(f"   Extended cases (L6-L9): 140")
print(f"   Total expected: ~479 lines")
