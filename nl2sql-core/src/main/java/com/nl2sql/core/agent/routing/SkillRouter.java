 ...
 * - 低置信度 → 降级到完整 Plan-and-Execute（FALLBACK）
 */
@Slf4j
@Component
public class SkillRouter {
    ...
    public void init() {
        ...
        // COMPLEX 意图 → 所有 Skills（LLM 辅助决策 + Workflow 编排）
    ...
    private RoutingResult decideRoutingStrategy(IntentClassification intent, Long datasourceId) {
        ...
        // 规则 1: 高置信度（>= 0.8）且非 UNKNOWN/COMPLEX → 直接路由
        ...
        // 规则 2: COMPLEX 意图或中等置信度（0.5-0.8）→ LLM 辅助决策（实际走 Plan-and-Execute）
        if (intentType == IntentType.COMPLEX || confidence >= 0.5) {
            ...
            return RoutingResult.llmAssisted(
                skills,
                String.format("%s意图 (confidence=%.2f)，使用 Plan-and-Execute", intentType, confidence),
                intent
            );
        }

        // 规则 3: 低置信度（< 0.5）→ 降级到完整 Plan-and-Execute
        return RoutingResult.fallback(
            String.format("低置信度意图 (confidence=%.2f)，降级到 Plan-and-Execute", confidence),
            intent
        );
    }
