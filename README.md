...
> DataMind AI 的核心设计理念不是"让 LLM 自由发挥"，而是通过工程化手段 **Harness（驾驭）** 其能力边界。

我们将 LLM 视为"强大但不可控的引擎"，通过三层机制实现生产级可靠性：

| 层级 | 机制 | 核心实现 | 解决的问题 |
|------|------|---------|-----------|
| **约束层**<br/>*Constraint* | 限制 LLM 的决策空间，防止越界 | Schema 白名单校验 / SQL 安全 AST 解析 / 参数验证器 / 高风险人工审批 / 五层安全防护 | 幻觉列名、危险操作、格式错误、数据越权 |
| **增强层**<br/>*Augmentation* | 弥补 LLM 的能力短板，注入领域知识 | 三级缓存 / 双层 RAG / CrossEncoder 重排 / 行业 SPI 扩展 / 负面 Prompt 注入 / 低分示例拦截 | 重复查询浪费、行业术语不理解、同类错误重犯 |
| **编排层**<br/>*Orchestration* | 控制 LLM 的调用时机，避免过度依赖 | 复杂度评估路由 / 迭代式表发现 / 单数据源快速路径 / Self-Correction 循环 / Plan-and-Execute 架构 | 大小模型一刀切、过度调用 LLM、一次生成即最终 |

这种 **"约束 + 增强 + 编排"** 的设计，使得系统在保持 LLM 灵活性的同时，达到了企业级应用所需的 **确定性、安全性和可观测性**。
