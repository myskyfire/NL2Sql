...
public class SQLRiskAnalysisTool {
    ...
    public String analyzeSQLRisk(String sql, Long datasourceId) {
        try {
            log.info("[SQLRiskAnalysisTool] 开始分析SQL风险: datasourceId={}", datasourceId);
            
            if (datasourceId == null) {
                return "❌ 缺少数据源ID，无法执行风险分析";
            }
            
            SQLRiskAnalyzer.RiskAnalysisResult result = riskAnalyzer.analyzeRisk(sql, datasourceId);
