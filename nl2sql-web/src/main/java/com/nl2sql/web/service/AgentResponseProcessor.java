...
public class AgentResponseProcessor {
     ...
     * @return 处理后的响应
     */
    public Map<String, Object> processResponse(String agentResponse, Map<String, Object> request, Map<String, Object> userInfo) {
        try {
            // 1. 清理 Markdown
     ...
     * 处理 JSON 响应
     */
    private Map<String, Object> handleJsonResponse(String jsonResponse, Map<String, Object> request, Map<String, Object> userInfo) throws Exception {
     ...
     * 处理纯文本响应
     */
    private Map<String, Object> handleTextResponse(String text, Map<String, Object> request, Map<String, Object> userInfo) {
        ...
        if (text.contains("推荐使用") || text.contains("🎯")) {
            ...
            if (tableName != null) {
                ...
                Long currentDsId = request.get("datasourceId") != null ? ((Number) request.get("datasourceId")).longValue() : null;
                ...
                if (!availableIn.isEmpty()) {
                    String suggestion = String.format("\n\n💡 **提示**：表 `%s` 可能在以下数据源中存在：%s\n是否要切换到这些数据源重新查询？", tableName, String.join("、", availableIn));
    ...
    private Long extractDatasourceIdFromText(String text) {
        try {
            ...
            if (matcher.find()) {
                ...
                List<Map<String, Object>> results = jdbcTemplate.queryForList("SELECT id FROM datasource_config WHERE name = ? AND is_active = 1", dsName);
    ...
    private String extractTableNameFromError(String errorMessage) {
        ...
        try {
            ...
            Pattern[] patterns = {Pattern.compile("Table ['\"]([^'\"]+)['\"]"), Pattern.compile("表 ['\"]?([a-zA-Z_][a-zA-Z0-9_]*)['\"]?"), Pattern.compile("table ['\"]?([a-zA-Z_][a-zA-Z0-9_]*)['\"]?")};
